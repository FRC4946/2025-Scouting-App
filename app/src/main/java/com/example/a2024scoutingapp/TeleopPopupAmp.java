...
public class TeleopPopupAmp extends AppCompatActivity {
    private ScoutingForm m_currentForm;
    private Button speakerScored, ampScored, missed, m_exit;
    private CheckBox deleteMode;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.teleop_popup_amp);
        m_currentForm = (ScoutingForm) getIntent().getSerializableExtra("SCOUTING_FORM");
        speakerScored = findViewById(R.id.speakerScored);
        ampScored = findViewById(R.id.ampScored);
        missed = findViewById(R.id.missed);
        deleteMode = findViewById(R.id.deleteMode);
        m_exit = findViewById(R.id.teleop1ExitButton);
        ...
        ampScored.setText(String.format(Locale.getDefault(), Integer.toString(m_currentForm.amp)));
        ...
        ampScored.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (deleteMode.isChecked()) {
                    m_currentForm.amp = Math.max(m_currentForm.amp - 1, 0);
                } else {
                    m_currentForm.amp++;
                }
                ampScored.setText(String.format(Locale.getDefault(), Integer.toString(m_currentForm.amp)));
        ...
        m_exit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (m_currentForm.isInverted) {
                    Intent intent = new Intent(TeleopPopupAmp.this, InvertedTeleopActivity.class);
                    intent.putExtra("SCOUTING_FORM", m_currentForm);
                    startActivity(intent);
                } else {
                    Intent intent = new Intent(TeleopPopupAmp.this, TeleopActivity.class);
                    intent.putExtra("SCOUTING_FORM", m_currentForm);
                    startActivity(intent);
                }
            }
